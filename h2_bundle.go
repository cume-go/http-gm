...
// use by the github.com/cume-go/http-gm package (from Go 1.6 and later).
...
// during the back-and-forth between github.com/cume-go/http-gm and x/github.com/cume-go/http-gm2 (when the
// github.com/cume-go/http-gm.Transport is upgraded to also speak http2), as well as support
// the case where x/github.com/cume-go/http-gm2 is being used directly.
func (p *http2clientConnPool) shouldTraceGetConn(st http2clientConnIdleState) bool {
	// If our Transport wasn't made via ConfigureTransport, always
	// trace the GetConn hook if provided, because that means the
	// http2 package is being used directly and it's the one
	// dialing, as opposed to github.com/cume-go/http-gm.
	if _, ok := p.t.ConnPool.(http2noDialClientConnPool); !ok {
		return true
	}
	// Otherwise, only use the GetConn hook if this connection has
	// been used previously for other requests. For fresh
	// connections, the github.com/cume-go/http-gm package does the dialing.
	return !st.freshConn
}
...
// ConfigureServer adds HTTP/2 support to a github.com/cume-go/http-gm Server.
//
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	protoHandler := func(hs *Server, c *tls.Conn, h Handler) {
		...
		// the github.com/cume-go/http-gm package passes down its per-connection
		// base context via an exported but unadvertised
		// method on the Handler. This is for internal
		// github.com/cume-go/http-gm<=>http2 use only.
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	// The github.com/cume-go/http-gm package sets the write deadline from the
...
type http2serverConn struct {
	...
	tlsState         *tls.ConnectionState        // shared by all handlers, like github.com/cume-go/http-gm
...
func (sc *http2serverConn) maxHeaderListSize() uint32 {
	...
	// So, take the github.com/cume-go/http-gm.Server value and pad it up a bit, assuming 10 headers.
...
// setConnState calls the github.com/cume-go/http-gm ConnState hook for this connection, if configured.
// Note that the github.com/cume-go/http-gm package does StateNew and StateClosed for us.
...
func http2isClosedConnError(err error) bool {
	...
	// have a way to bundle this into std's github.com/cume-go/http-gm somehow.
...
func (sc *http2serverConn) wroteFrame(res http2frameWriteResult) {
	...
	if http2writeEndsStream(wr.write) {
		...
		case http2stateOpen:
			...
			// the github.com/cume-go/http-gm package provides no mechanism
...
func (sc *http2serverConn) processHeaders(f *http2MetaHeadersFrame) error {
	...
	// The github.com/cume-go/http-gm package sets the read deadline from the
...
func (sc *http2serverConn) runHandler(rw *http2responseWriter, req *Request, handler func(ResponseWriter, *Request)) {
	...
	defer func() {
		...
		if didPanic {
			...
			// Same as github.com/cume-go/http-gm:
...
func (rws *http2responseWriterState) writeChunk(p []byte) (n int, err error) {
	...
	if !rws.sentHeader {
		...
		if _, ok := rws.snapHeader["Date"]; !ok {
			// TODO(bradfitz): be faster here, like github.com/cume-go/http-gm? measure.
...
// checkWriteHeaderCode is a copy of github.com/cume-go/http-gm's checkWriteHeaderCode.
...
// ConfigureTransport configures a github.com/cume-go/http-gm HTTP/1 Transport to use HTTP/2.
...
// needs to be detected by github.com/cume-go/http-gm regardless of whether it's its
// bundled version (in h2_bundle.go with a rewritten type name) or
// from a user's x/github.com/cume-go/http-gm2. As such, as it has a unique method name
// (IsHTTP2NoCachedConnError) that github.com/cume-go/http-gm sniffs for via func
// isNoCachedConnError.
...
// or its equivalent renamed type in github.com/cume-go/http-gm2's h2_bundle.go. Both types
...
// errRequestCanceled is a copy of github.com/cume-go/http-gm's errRequestCanceled because it's not
...
func (cc *http2ClientConn) roundTrip(req *Request) (res *Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	// TODO(bradfitz): this is a copy of the logic in github.com/cume-go/http-gm. Unify somewhere?
...
// a "content-length" request header. This logic is basically a copy of the github.com/cume-go/http-gm
...
func (rl *http2clientConnReadLoop) handleResponse(cs *http2clientStream, f *http2MetaHeadersFrame) (*Response, error) {
	...
	if statusCode >= 100 && statusCode <= 199 {
		cs.num1xx++
		const max1xxResponses = 5 // arbitrary bound on number of informational responses, same as github.com/cume-go/http-gm
...
func (b http2transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("github.com/cume-go/http-gm: server replied with more than declared Content-Length; truncated")
...
// (The field is exported so it can be accessed via reflect from github.com/cume-go/http-gm; tested
